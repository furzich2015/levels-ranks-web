...
namespace app\modules\module_page_sb\ext\qb;
...
class Pdox implements PdoxInterface
{
    ...
    public function __construct()
    {
        ...
        if(!empty($db['SourceBans'][0])):

            $this->prefix = $db['SourceBans'][0]['DB'][0]['Prefix'][0]['table'];
            $this->cacheDir = __DIR__ . '/cache/';
            $this->debug = true;

            $dsn = 'mysql:host=' . $db['SourceBans'][0]['HOST'] . ';port=' . $db['SourceBans'][0]['PORT'] . ';dbname=' . $db['SourceBans'][0]['DB'][0]['DB'];

            try {
                $this->pdo = new PDO($dsn, $db['SourceBans'][0]['USER'], $db['SourceBans'][0]['PASS']);
